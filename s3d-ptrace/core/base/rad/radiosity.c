...
static void initialize(int n, Color *m, Color *dm, Real * a, Hpoly **p, Color *e)
{
  int i;
  for (i = 0; i < n; i++) {
    a[i] = hpoly3_area(p[i]);
...
static Real visible(int n, Hpoly **p, Vector3 v, Vector3 d)
{
  Ray r = ray_make(v, d);

  while (n--) {
    Real t = hpoly3_ray_inter(p[n], hpoly3_plane(p[n]), r);
    if (t > REL_EPS && t < 1)
      return 0.0;
  }
  return 1.0;
...
static Real formfactor(int i, int j, int n, Hpoly **p, Real *a)
{
  Vector3 vi, vj, vji, d;
  Real r2, ci, cj;
  ...
  if ((cj =  v3_dot(hpoly_normal(p[j]), d)) < REL_EPS)
    return 0;
  if ((ci = -v3_dot(hpoly_normal(p[i]), d)) < REL_EPS)
    return 0;
...
Color *radiosity_prog(int n, Hpoly **p, Color *e, Color *rho)
{
  int src, rcv, iter = 0;
  Real ff, *a = NEWTARRAY(n, Real);
