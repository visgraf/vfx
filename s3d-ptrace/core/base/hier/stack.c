...
Vector4 s4_v4xform(Stack4 *s, Vector4 v)
{
  return v4_m4mult(v, *s->mtop);
}
...
{
  *s->mtop = m4_m4prod(*s->mtop, *md );
  if ( im == (Matrix4 *)0)
    *s->itop = m4_m4prod(m4_inverse(*md), *s->itop);
  else
    *s->itop = m4_m4prod(*im, *s->itop);
}
